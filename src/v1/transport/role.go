...
type roleT struct {
	business abstraction.RoleB
	httpUtil util.HttpUtil
}

func NewRoleT(appCtx config.AppContext) roleT {
	business := business.NewGormRoleB(appCtx)
	return roleT{business: business, httpUtil: util.NewHttpUtil()}
}

func (t roleT) Save(appCtx config.AppContext) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		var creation domain.RoleCreation
...
func (t roleT) FindById(appCtx config.AppContext) gin.HandlerFunc {
	return func(ctx *gin.Context) {
...
func (t roleT) FindAll(appCtx config.AppContext) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		var page dto.Page
...
func (t roleT) FindAllBy(appCtx config.AppContext) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		var page dto.Page
		if parseErr := ctx.ShouldBindQuery(&page); parseErr != nil {
			t.httpUtil.DoErrorParseQuery(ctx, parseErr)
...
func (t roleT) FindAllArchived(appCtx config.AppContext) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		var page dto.Page
		if parseErr := ctx.ShouldBindQuery(&page); parseErr != nil {
			t.httpUtil.DoErrorParseQuery(ctx, parseErr)
...
func (t roleT) Update(appCtx config.AppContext) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		var update domain.RoleUpdate
		id, parseErr := strconv.Atoi(ctx.Param("id"))
		if parseErr != nil {
			t.httpUtil.DoErrorGetPath(ctx, "id")
			return
...
func (t roleT) Delete(appCtx config.AppContext) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		id, parseErr := strconv.Atoi(ctx.Param("id"))
		if parseErr != nil {
			t.httpUtil.DoErrorGetPath(ctx, "id")
